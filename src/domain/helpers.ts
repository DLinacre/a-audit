...
import { CategoryItem, RoleItem, BuilderState } from '../types';
...
export function relevantCategories(typeId: string): CategoryItem[] {
  return CATEGORIES.filter(c => typeApplies((c as any).types || c.appliesTo, typeId));
}

export function relevantRoles(typeId: string): RoleItem[] {
  return ROLES.filter(r => typeApplies((r as any).types || r.appliesTo, typeId));
}
...
export function gatherDyn(typeId: string, state: BuilderState): Record<string, string> {
  const t = TYPES[typeId] || TYPES.generic;
  const out: Record<string, string> = {};
  const fields = t.fields || [];
  for (const f of fields) {
    const val = (state as any)[f.id];
    if (cleanRaw(val)) out[f.id] = cleanRaw(val);
  }
  return out;
}
